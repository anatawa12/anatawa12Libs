 ...
 */
class MappedResultSet<V>(val base: Set<V>, private val transform: (V) -> V) : Set<V> {
 ...
 * result is transformed by [transform]
 */
class MappedResultMutableSet<V>(val base: MutableSet<V>, private val transform: (V) -> V) : MutableSet<V> {
	override val size: Int
		get() = base.size
